...
def file2matrix(filename):
    ...
    fr=open(filename)
    arrayOLones=fr.readlines()
    numberOfLines=len(arrayOLones)
    returnMat=zeros((numberOfLines,3))
    classLabelVector=[]
    index=0
    for line in arrayOLones:
        line=line.strip()
        lineFromLine=line.split('\t')
        returnMat[index,:]=lineFromLine[0:3]
        classLabelVector.append(lineFromLine[-1])
...
def classifyPerson():
    ...
    percentTats=float(raw_input("percentage of time spent playing wedio games?"))
